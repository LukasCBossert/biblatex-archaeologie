...
\def\thisarchaeologiedate{2016-09-XX}
\input{archaeologie-preamble.tex}
\NewEnviron{bibbsp}[1]
  {\par\medskip\noindent
  \begin{tikzpicture}
    \node[inner sep=0pt,label={[label distance=.5em,xshift=-4.5cm]above:\normalsize\textbf{\Cref{#1}}}] (box) {\parbox[t]{\textwidth}{%
    \hspace{.025\textwidth}\begin{minipage}{.95\textwidth}
     \footnotesize \BODY 
      \end{minipage}\hspace{.025\textwidth}}
    };
    \draw[codegreen,line width=2pt] 
...
\alertinfo{Activating |inreferences=true| causes the cited |@Inreference| entry to be automatically \emph{omitted} in the (final) bibliography,
...
\parbox[t]{2.3cm}{Wulf-Rheidt 2013} \parbox[t]{9.5cm}{%
...
\begin{example}[lefthand width=5cm]{}
\citeauthor{Mann2011,Hufschmid2010}
\end{example}
...
When we cite \citeauthor{Vitr} by typing 
\begin{lstlisting}
\cite[1,1,2]{Vitr}.
\end{lstlisting}
we get this result:
...
If you cite these works you do as usual:
\begin{lstlisting}
\cite[1,1,2]{Vitr}; see as well \cite{Vitr:Loeb};
with changed translation in \cite{Vitr:Krohn} 
and in \cite{Vitr:Saliou}; 
in \cite{Vitr:Fischer} it is kept untranslated.
\end{lstlisting}
we get this result:
\begin{bsp}
\cite[1,1,2]{Vitr}; see as well \cite{Vitr:Loeb};
...
\alertinfo{Notice that the field |sortshorthand| should be filled with the sorting order you would like to have in the bibliography for the varient works by \citeauthor{Vitr}, see below.}

\printbib[6.5em]{Vitr,Vitr:Fischer,Vitr:Krohn,Vitr:Loeb,Vitr:Saliou}
...
\alertinfo{If you want to separate author-year labels from |shorthand| labels in your bibliography,  
...
\printbib[9.5em]{Bossert:forthcoming}
...
\alertwarning{If you have the option \texttt{initials} activated your index will have some issues with names that are being shortend automatically.\label{initials:index}
We are working on it, see the issue on \href{https://github.com/LukasCBossert/biblatex-archaeologie/issues/97}{GitHub} and on \href{http://tex.stackexchange.com/q/330971/98739}{\TeX.sx} for more information.\footnotemark}
\footnotetext{So far we can only offer a work-around:
