...
Ansonsten kleine Korrekturen zur Anpassung an die Vorgaben des Deutschen Archäologischen Instituts.
...
%\begin{itemize}
%\item Die Verwendung von |maintitle| wurde auf |booktitle| (bei Einträgen von |@in...|) umgestellt.
%\item Der Rufname antiker Autoren wird nicht mehr unter |namea| sondern unter |shortauthor| eingetragen.
%\item Die Eintrags-Option |unbekannt| fällt weg; es reicht wenn der Eintrag für Autor/Editor freibleibt und dafür das Feld |label| ausgefüllt wird.
%\item Die Präambel-Option |miturl| fällt weg; es wird nun eine URL/DOI/etc. angezeigt, wenn dies im Bibliographieeintrag entsprechend eingetragen ist.
%\item Einfügung der Möglichkeit Werke miteinander zu verknüpfen (|related|) (\cref{book})\\ Dies kommt vor allem bei |@review| zum tragen, wo man das rezensierte Werk nicht mehr über |titleaddon| einfügt (\cref{review}).
%\item Umbenennung der Eintrags-Option |nurshorthand| in |corpus| (\cref{corpus})
%\end{itemize}
...
\item[lexika]  Bibliographieeinträge mit |Options={lexikon}| werden bei aktivierter Funktion vollreferenziert geschrieben. Siehe \cref{lexika}.
...
Diese Eintragsoption ist für Lexikoneinträge (|@inreference|; siehe \cref{inreference}) sinnvoll, wenn diese zu den ›gängigen Lexika‹ gehören (RE, DNP, LTUR, LIMC, etc.).
...
|  Booktitle = ThesCRA-kurz,      |\emph{Verwendung von @string}\\
|  Pages   = {11--16},|\\
|  Volume  = {8},|\\
|  Year    = {2012},|\\
|  related={ThesCRA},|\\
...
Zusätzlich ist der Eintrag über |related={ThesCRA}| mit dem dazugehörigen Lexikon (|@reference|) verknüpft, was für die Auflösung der Abkürzung (ThesCRA) hilfreich ist.
...
Diese Option ist vorallem in Kombination mit der Paketoption |lexika| (siehe \cref{lexika}) relevant, da  sich die Zitation in der Fußnote ändert und der Eintrag in der Bibliographie nicht aufgeführt wird (da er in der Fußnote vollreferenziert ist).
...
Da im Bibliographieeintrag |options={lexikon}| (siehe \cref{lexikon}) geschrieben wurde, werden bei gewählter Präambel-Option |lexika=true| alle Lexikoneinträge von der Bibliographie ausgeschlossen, da sie ja in der Fußnote vollzitiert werden.
...
\begin{refsection}
\begin{bsp}
\nocite{Sear_2006,Taylor_2008,Welch_2007}
\printbibliography[title={Forschungsliteratur}]
\end{bsp}
 \end{refsection}
