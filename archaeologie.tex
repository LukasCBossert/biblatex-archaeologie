...
\end{bsp}
Nevertheless the advantage of this list lies in the possibility to create a separate bibliography with all the abbreviations of used journaltitles and series (see \cref{bibliographie}) without being prone to and typing differences and errors.

However, if a journal or a series is \emph{not} included in the list (\cref{listen}) 
then this journal/series will \emph{not} be abbreviated and converted to full title in curly brackets in the respective field e.g. |journaltitle=|\marg{title of the journal} or |series=|\marg{name of the series}. Therefore the field content will not be printed. At least BibTeX gives a warning in its log which can be checked.\footnote{For example something like \enquote{\enquote{journaltitle} in entry \enquote{<entrykey>} cannot be null, deleting it}.}
