...
To make it clear that the book or collection is a so-called ›Festschrift‹m a commemorative volume, or a exhibition or auction catalogue you need an additional note to make it clear.
...
\printbibliography[notkeyword=Sigel]
\end{bsp}
 \end{refsection}
...
What you need are two separate entries: one as a |@Review| the other is a |@Book|, |@Collection|, |@Proceeding| or something else.
...
For (oral) given papers e.g. at a colloquium or a proceeding we created a new entry type calld ›@Talk‹.
...
\DescribeMacro{eprint}\DescribeMacro{eprinttype} If you have a paper or book which is available in the internet via a permanent link there a different possibilities referring to it --  see also the biblatex-documentation 3.11.7.
\begin{description}
\item[JSTOR] If you can refer to the online plattform \href{www.jstor.org}{jstor} then you need the individual number for the article -- cf. \cref{eprint:jstor}:
\begin{lstlisting}
...
In the following example we use an enty type |@Article|:
\begin{lstlisting}
@Article{Cosa_1949,
  title        = {Cosa},
...
The bold entry on the left is the |bibtexkey|.
...
Generell gilt, dass Akzente weggelassen wurden, Umlaute ausgeschrieben (ä--> ae), ähnliche Buchstaben mit Äquivalenten ersetzt wurden (ı --> i).
...
\DescribeMacro{noabbrv}Die Ausgabe erfolgt über die Option |noabbrv|.
%\begin{multicols}{1}
