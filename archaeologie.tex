...
LCB: Ich habe nicht gefunden, wie das geht, dass ein bool am Anfang geladen werden kann. }
...
"		" "dai=true,						% true=Bibliographie und Zitation nach DAI-Vorgaben"\\
"		" "lexika=true, 				%true =  bspw. LTUR 2 (1994) 123 s. v."\\
"		" "]{biblatex}"
 
In diesem Beispiel wurden die "archaeologie"-Optionen "dai" und "lexika" geladen ("=true"). 
...
Wenn die Option "dai" geladen wird, werden automatisch weitere für den Zitations- und Bibliographiestil notwendige Optionen geladen, sodass diese nicht manuell geladen werden müssen.
Diese sind:

%\begin{bsp}
 "autorjahr		% Zitation nach Autor-Jahr-System"\\
 "hrsg				% (Hrsg.) anstatt 'hrsg. v.'"	\\
 "nurinit			%Initiale des Vornamens und Nachname in Bibliographie"\\
 "mitjahr			% Angabe des Jahres nach Autor/Hrsg."	\\
  "jahrkeineklammern			%Keine Klammern um Jahreszahlen"\\
 "initialnachname		% bei \citeauthor"\marg{Schlüssel}" nur Initial und Nachname"\\
...
Wenn diese Option aktiviert wird, erfolgt keine Angabe von eines Originaltitels und des Übersetzers des Werkes. Dies gilt allerdings nicht, wenn  man die zuvor genannte Option "dai" geladen hat.
...
Die Darstellung der Namen kann auf drei Arten geschehen und sind stets mit ihrem Bibliographieeintrag zum Bibliographie-\marg{Schlüssel} via "hyperref" verlinkt. Die drei Arten sind: mit dem vollen Vor- und Zunamen, nur mit dem Nachnamen oder mit den Initialen des Vornamens und dem Nachnamen. In einer Fußnote jedoch wird stets nur der/die Nachnamen gesetzt.
...
\item\label{name:c}  \DescribeMacro{initialnachname}\footnote{Diese Option ist standardmäßig mit "dai" geladen.}
\ldots , dies behaupten ebenso  R. M. Boehmer und N. Wrede in ihrem jüngsten Werk \emph{Astragalspiele in und um Warka} (1985).
 \end{enumerate}
\end{bsp}


Werden allerdings mit "\citeauthor", bzw. "\citetitle"  antike Autoren und ihre Werktitel aufgerufen (dafür muss im Bibliographieeintrag "options=antik" geschrieben werden), dann wird für den Autorenname das Feld "usera" ausgelesen, in dem der deutsche Rufnamen des Autors steht. 
...
\ldots Auch Quintilian nennt in \emph{Ausbildung des Redners} die notwendigen phsysischen Qualitäten eines Redners. 
\end{bsp}
...
* * *
\end{center}
\DescribeMacro{shortjournal}
Mit dieser Option werden Artikel mit abkürzten Zeitschriftennamen in der Bibliographie angegeben.
Dafür wird das Feld "shortjournal" ausgelesen. Gibt es keine Abkürzung, also wurde das Feld "shortjournal" leer gelesen, wird automatisch das Feld "journal" ausgelesen.
Die Option ist automatisch im Stil "dai" und "dai-verweis" geladen. Für die Stile "autorjahr" und "kurztitel" kann sie optional aktiviert werden.



 \section[Beschreibung]{Beschreibung\protect\footnote{Die Beispiele sind teilweise aus der PDF des DAI entnommen \mkbibparens{\url{https://www.dainst.org/documents/10180/70593/01_Schlagwortliste_quer.pdf}\\
...
\begin{bsp}
Fest.\hspace{3em}
W. M. Lindsay, De verborum significatu quae supersunt cum Pauli epitome, Bibliotheca scriptorum et Graecorum et Romanorum Teubneriana (Leipzig 1913; Nachdr. Leipzig 1965)
\end{bsp}
