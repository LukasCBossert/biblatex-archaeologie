...
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}
 ...
 \captionsetup[lstlisting]{format=listing,singlelinecheck=false, margin=0pt, font={sf},size=footnotesize}
...
\begin{lstlisting}[label=Koyunlu1990,caption={{@}Article\{Koyunlu1990,…\} }]
@Article{Koyunlu1990,
  author       = {Koyunlu, A.},
...
The bibliography-entry looks like this
 \begin{lstlisting}[label=Apul:met,caption={{@}Book\{Apul:met,…\} }]
@Book{Apul:met,
...
This bibliographical entry |Lefebvre_2011| contains following fields:
 \begin{lstlisting}[label=Lefebvre2011,caption={{@}Book\{Lefebvre2011,…\} }]
@Book{Lefebvre_2011,
...
\begin{lstlisting}[label=Lefebvre1977,caption={{@}Article\{Lefebvre1977,…\} }]
...
\begin{lstlisting}[label=Nieddu1995,caption={{@}Inreference\{Nieddu1995,…\} }]
...
\end{lstlisting}
  \begin{refsection}
There are two ways of the display of |\cite{Nieddu_1995}|:
   \begin{bsp}
...
\end{lstlisting}\label{jstor}\label{doi2}
...
\end{lstlisting}\label{jstor1}\label{zenon2}
looks like this
...
\end{lstlisting}\label{lstlocations-munich}
...
\end{lstlisting}\label{lstlocations-leipzig2}
...
\end{lstlisting}\label{lstlocations-berlin}
...
The first edition of \citetitle*{Zanker_2009}  by \citeauthor*{Zanker_2009}  has been published in 1987:
\begin{lstlisting}
@Book{Zanker_2009,
  author        = {Zanker, Paul},
...
\end{lstlisting}\label{zenon}\label{lstlocations-munich}\label{lstlocations-leipzig}
In \citeyear{Zanker_1988}, one year later  it was translated by A. H. Shapiro:
\begin{lstlisting}
@Book{Zanker_1988,
  author      = {Zanker, Paul},
...
The translated book |Zanker_1988| is connected with the book |Zanker_2009| via the field |related = |\marg{bibtex-key} and the relation got specified with the field |relatedtype|, in this case it is a translation so |={translationof}|.

You don’t have to cite |Zanker_2009| to have the information visible in the bibliography. 
It will be included automatically. For the following bibliography we only use |\cite{Zanker_1988}|:

\printbiball[5em]{Zanker_1988}
...
\end{lstlisting}\label{lstlocations-mainz}
...
\end{lstlisting}\label{psq}\label{lstlocations-tubingen}
...
\end{lstlisting}\label{doi}
In line 5 and 6 you can also write the full or abbrivated journal title in the fields |journaltitle| or |shorttitle| (e.g. |journaltitle = {British School of Athens}|, |shortjournal = {BSA}|), but we chose to use a |@string| (cf. \cref{list-ancient,string}) again.
\printbib[7em]{Evangelidis_2014}
%\begin{refsection}\end{refsection}

Sometimes you know the author of an article and he sends you the proof of his paper beforehand.
You can also cite this version of the paper and provide information about the publication status in the field |pubstate|.
...
\printbib[8em]{Bossert_forthcoming}
...
\end{lstlisting}
The |postnote| (\emph{930 Nr. 283}) will be printed instead of the |pages|:
 \begin{bsp}
...
\end{lstlisting}
You maybe noticed that the reveiw (|Bell_2011|) is with the field |related| in line 8 connected to the entry |Welch_2007|.
...
The following entry is an example reviews the book by \citeauthor*{Welch_2007}, too but with an individual title:
\begin{lstlisting}
...
In his review
\citeauthor{Taylor_2008} not only describes the book called
 \citetitle{Welch_2007} by \citeauthor{Welch_2007}, 
 but at the same time compares it with \citeauthor{Sear_2006}s \citetitle{Sear_2006}.

The entry of the first analysed book \citetitle{Welch_2007} has been described in \cref{review}.
...
\end{lstlisting}
In the field |related| you can have several |bibtex-keys| which has to be separated by a comma (see line 7).
...
\begin{bsp}
\printbiblist[title={Abbreviation of series}]{shortseries}\end{bsp}
...
Just write the first page in the \oarg{postnote} and then the \cs{psq} -- cf. \cref{psq}.
...
\DescribeMacro{jstor}
 If you can refer to the on-line plattform \href{www.jstor.org}{jstor} then you need the individual number for the article -- cf. \cref{jstor}:
\begin{lstlisting}
...
\DescribeMacro{urn}
For all papers referable via an ›urn‹ (\emph{Uniform Resource Name}), which have been registered at the German National Library -- cf. \cref{urn}
\begin{lstlisting}
eprint = *@\marg{urn:xxx}@*
eprinttype = {urn} 
\end{lstlisting}			
...
You only need to insert the individual Zenon-number of the entry, e.g. \emph{http://zenon.dainst.org/Record/001371402} where \emph{001371402} is the individual number -- cf. \cref{zenon}.
\begin{lstlisting}
eprint = *@\marg{Zenon-number}@*
eprinttype = {zenon} 
\end{lstlisting}			
OR  cf. \cref{zenon1,zenon2}.
\begin{lstlisting}
zenon = *@\marg{Zenon-number}@*
\end{lstlisting}			


\DescribeMacro{doi} In addition you can also refer to a document via its |doi|-number -- cf. \cref{doi,doi2} 
\begin{lstlisting}
doi = *@\marg{doi-number}@*
\end{lstlisting}	
...
\begin{lstlisting}
\parencite[vgl.][930 Nr. 283]{Neils_1994}.
\end{lstlisting}
...
The bold entry on the left is to use for |location=|\meta{location} -- do not put \meta{location} into |{}|, e.g. \cref{lstlocations-munich,lstlocations-leipzig,lstlocations-leipzig2,lstlocations-tubingen,lstlocations-mainz}
\begin{multicols}{2}
\begin{description}
