...
%Neben internen Änderungen an der Programmierung und der Auslagerung verschiedener Dateien  hat sich vor allem folgendes verändert:
\begin{itemize}
\item Die Verwendung von |lexikon| als Eintragsoption fällt weg, da alle Lexika gleich behandelt werden (\cref{inreference}).

\item Hinzufügen der |cite|-Befehle |cites| und |parencites| (\cref{cite-befehle}).

\item Hinzufügen der Präambel-Option |width| (\cref{width})
\end{itemize}
...
\newpage\section{Use -- Verwendung}
 \DescribeMacro{archaeologie} \begin{multicols}{2}%
This style is called |archaeologie| and has to be loaded in the preamble of your file.

\begin{syntax} |\usepackage[style=archaeologie,%|\\
 |            |\meta{more options}|]{biblatex}|\\
 |\bibliography|\marg{|bib|-file}\end{syntax}\par
 
You can use more of the  \enquote{konventionellen} |biblatex|-options or use the options of |archaeologie| -- as described below.

By default |archaeologie| is loading the citation style of the German Archaeology Institute (DAI) with the author-year-system.
If you would like to use this system as it is, you don’t have to do anything else.

Do not forget the code for the  |\printbibliography|-command, so you will get a bibliography.
...
Since |archaeologie|supports different citation styles regarding different kind of  opuses it is advised to show this differentiation with different bibliographies. 
There are various possibilities to do that we show a fast and easy way to do so  (\cref{bibliographie}). \\
 \columnbreak\selectlanguage{ngerman}
...
\begin{multicols}{2}
 \begin{description}
\item[edby] In |incollections| the editor is now shown after the book title.   \cref{edby}. \columnbreak
\foreignlanguage{ngerman}{ \item[edby] Bei Sammelbänden steht anstatt \enquote{Hrsg.} nun \enquote{hrsg. v.} nach dem Buchtitel.  \cref{edby}.}
\end{description}
\end{multicols}
\begin{multicols}{2}
 \begin{description}
 \item[yearseries] Die Reihe wird erst nach der Jahreszahl ausgegeben.  \cref{yearseries}.\columnbreak
 \foreignlanguage{ngerman}{ \item[yearseries] Die Reihe wird erst nach der Jahreszahl ausgegeben.  \cref{yearseries}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
 \item[yearinparens] Die Jahreszahl wird in Klammern gesetzt.  \cref{yearinparens}.\columnbreak
\foreignlanguage{ngerman}{  \item[yearinparens] Die Jahreszahl wird in Klammern gesetzt.  \cref{yearinparens}.}
  \end{description}
\end{multicols}

  \begin{multicols}{2}
 \begin{description}
\item[translation] Anzeige der Angaben des Originaltitels, Übersetzers und der Sprache, aus der übersetzt wurde. Bei Einträgen, die |options=antik| haben, ist dies Standard.  \cref{translation}. \columnbreak
\foreignlanguage{ngerman}{\item[translation] Anzeige der Angaben des Originaltitels, Übersetzers und der Sprache, aus der übersetzt wurde. Bei Einträgen, die |options=antik| haben, ist dies Standard.  \cref{translation}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[noabbrevs] Die Abkürzungen von Zeitschriften und Serien (|shortjournal|/|shortseries|) werden ausgeschrieben, wofür die Felder |journaltitle| und |series| ausgelesen werden.  \cref{noabbrevs}.\columnbreak
\foreignlanguage{ngerman}{\item[noabbrevs] Die Abkürzungen von Zeitschriften und Serien (|shortjournal|/|shortseries|) werden ausgeschrieben, wofür die Felder |journaltitle| und |series| ausgelesen werden.  \cref{noabbrevs}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[publisher] Angabe aller Verlagsorte und Verlag selbst. Ändert die Formatierung der Edition und Erstausgabe.  \cref{publisher}.\columnbreak
\foreignlanguage{ngerman}{\item[publisher] Angabe aller Verlagsorte und Verlag selbst. Ändert die Formatierung der Edition und Erstausgabe.  \cref{publisher}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[bibfullname] Schreibt die Autoren/-Herausgeber mit vollem Namen in der Bibliographie.  \cref{bibfullname}.\columnbreak
\foreignlanguage{ngerman}{\item[bibfullname] Schreibt die Autoren/-Herausgeber mit vollem Namen in der Bibliographie.  \cref{bibfullname}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[inreferences]  Bibliographieeinträge mit |@inreference| werden bei aktivierter Funktion vollreferenziert geschrieben. Siehe \cref{inreferences}.\columnbreak
\foreignlanguage{ngerman}{\item[inreferences]  Bibliographieeinträge mit |@inreference| werden bei aktivierter Funktion vollreferenziert geschrieben. Siehe \cref{inreferences}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[lastnames] Schreibt die Autoren/-Herausgeber, die über  |citeauthor|\-\marg{key}  im Fließtext aufgerufen werden, nur mit Nachnamen (= Erscheinen in Fußnoten).  \cref{lastnames}.\columnbreak
\foreignlanguage{ngerman}{\item[lastnames] Schreibt die Autoren/-Herausgeber, die über    |citeauthor|\-\marg{key} im Fließtext aufgerufen werden, nur mit Nachnamen (= Erscheinen in Fußnoten).  \cref{lastnames}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[fullnames]Schreibt die Autoren/-Herausgeber, die über |citeauthor|\-\marg{key} im Fließtext aufgerufen werden,  mit vollem Vor- und Nachnamen -- sofern diese im Bibliographieeintrag vorhanden sind.  \cref{fullnames}.\columnbreak
\foreignlanguage{ngerman}{\item[fullnames]Schreibt die Autoren/-Herausgeber, die über  |citeauthor|\-\marg{key} im Fließtext aufgerufen werden,  mit vollem Vor- und Nachnamen -- sofern diese im Bibliographieeintrag vorhanden sind.  \cref{fullnames}.}
\end{description}
\end{multicols}

\begin{multicols}{2}
 \begin{description}
\item[scshape] Die Namen in den Fußnoten werden in Kapitälchen gesetzt \cref{scshape}, ausgenommen sind Werke unbekannter Herkunft (\cref{unbekannt}) und Werke antiker Autoren (\cref{antik,frgantik}).\columnbreak
\foreignlanguage{ngerman}{\item[scshape] Die Namen in den Fußnoten werden in Kapitälchen gesetzt \cref{scshape}, ausgenommen sind Werke unbekannter Herkunft (\cref{unbekannt}) und Werke antiker Autoren (\cref{antik,frgantik}).}
\end{description}
\end{multicols}
...
\cref{width}.\columnbreak
\foreignlanguage{ngerman}{\item[width] Mit dieser Option wird in der Bibliographie der Abstand zwischen dem Label und dem Vollzitat nach Belieben gesteuert.
\cref{width}.}
\end{description}
\end{multicols}
...
\begin{center} * * * \end{center}
 ...
 \begin{syntax}\noindent|\cites|\oarg{prenote}\oarg{postnote}\marg{Schlüssel}\%\\
 ...
 Möchte man mehrere Literaturangaben in den Fußnoten in Klammern setzen, dann empfiehlt sich dies mittels\par
 \begin{syntax}\noindent|\parencites|\oarg{prenote}\oarg{postnote}\marg{Schlüssel}\%\\
 				\indent\oarg{prenote}\oarg{postnote}\marg{Schlüssel}\%\\%
 				\indent\oarg{prenote}\oarg{postnote}\marg{Schlüssel}\ldots
 				\end{syntax} zu tun.
 Dieser Befehl berücksichtigt die Klammerregelung, die besagt, dass runde Klammern innerhalb einer Klammerumgebung als eckigen Klammern geschrieben werden müssen.
...
\DescribeMacro{width}
In der Bibliographie richtet sich der Abstand zwischen dem Label (Autor und Jahr)  und der vollständigen bibliographischen Angabe nach der Länge der längsten |shorthand| (|shorthandwidth|), wie sie i.d.R. bei antiken Werken vorkommt. 

Verwendet man beispielsweise keine |shorthand| im Text, dann erfolgt keine ›zweispaltige‹ Ausgabe in der Bibliographie, da in der Zeile direkt unter dem Label die vollständige bibliographie Auflösung erscheint.
Andererseits ist es ebenso möglich, dass man eine sehr lange |shorthand| in der Bibliographie hat, dann ist der Abstand zwischen dem Label und der bibliographischen Auflösung sehr lange.

In einem solchen Falle kann der Abstand (|shorthandwidth|) selbst bestimmt werden.
Dafür gibt man die eigens gewählte Länge wie folgt an:
