...
%	url=false,
	%babel=other,
	% lexika=true, %true = in Fußnote Zitation nach DAI (LTUR 2 (19994} 123 s.v. ); false = Zitation nach Autor-Jahr
	%uebersetzung,
	%bibvollername,
	%keineabkuerzung,
	%verlag,
...
%\MakeShortVerb{\|}
 ...
 Die Entwicklung des Codes findet unter \url{https://github.com/giovannotti/biblatex-archaeologie} statt: Anmerkungen und Fehlermeldungen sind sehr willkommen.} }
...
Für Anmerkungen und Hilfe Dank an ›moewew‹ und Herbert Voß.
...
\item Hinzufügen der Präambel-Option |abstand| (\cref{Abstand})
\end{itemize}
...
\item[lexika]  Bibliographieeinträge mit |@inreference| werden bei aktivierter Funktion vollreferenziert geschrieben. Siehe \cref{lexika}.
...
\item[abstand] Mit dieser Option wird in der Bibliographie der Abstand zwischen dem Label und dem Vollzitat nach Belieben gesteuert.
\cref{Abstand}
...
Die Option |uebersetzung|  ist standardmäßig auf |=false| gesetzt. 
...
%  \end{bsp}
 ...
 \cite{Nieddu_1995}
 \item mit der Option |lexika| wird dies zu:
...
Verwendet man diese Option (|lexika|), dann werden die als |@inreference| gekennzeichneten Einträge automatisch von der (End-)Bibliographie ausgeschlossen,  da sie bereits in den Fußnoten vollständig bibliographiert sind.
...
\subsubsection{abstand}\label{Abstand}
\DescribeMacro{abstand}
...
Für die erste Variante bedarf es keine Änderung, präferiert man die zweite Variante, so muss in der Präambel die Option |lexika=true| gesetzt werden. \DescribeMacro{lexika=true}
...
Gleiches gilt auch für das Zitieren von mehreren Einträgen in Klammern: Dafür wird analog der Befehl |\parencites|\marg{Schlüssel} verwendet:
 \begin{bsp}
	\ldots\ |\parencites[vgl.][930 Nr. 283]{Neils_1994}[9]{Nieddu_1995}.|
 	...
 	bzw. mit der Präambel-Option |lexika=true|:
 	 \begin{bsp}
...
Da der Bibliographieeintrag ein Lexikonartikel ist (|@inreference|), werden bei gewählter Präambel-Option |lexika=true| alle Lexikoneinträge von der Bibliographie ausgeschlossen, da sie ja in der Fußnote vollzitiert werden.

Ansonsten  (Präambel-Option |lexika=false| = default)  sieht der Eintrag in der Bibliographie so aus:
